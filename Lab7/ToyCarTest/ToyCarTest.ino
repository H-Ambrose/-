...
{
  byte temp;
  if (Battery_Voltage < 740) //Flag_Stop置1或者电压太低关闭电机
  {
    temp = 1;
  ...
  else if (Flag_Stop == 1) //Flag_Stop置1或者电压太低关闭电机
  {
    temp = 1;
    digitalWrite(RightP, LOW);  //电机驱动的电平控制
    analogWrite(RightN, 180);  //电机驱动的电平控制
    digitalWrite(LeftP, LOW);   //电机驱动的电平控制
    analogWrite(LeftN, 180);   //电机驱动的电平控制
  }
  else if(Flag_Stop == 2){
    temp = 1;
    digitalWrite(RightN, LOW);  //电机驱动的电平控制
    analogWrite(RightP, 180);  //电机驱动的电平控制
    digitalWrite(LeftN, LOW);   //电机驱动的电平控制
    analogWrite(LeftP, 180);   //电机驱动的电平控制    
  }
  else  temp = 0;
  return temp;
}
...
void setAcc(void)
{
  ...
  MotorL = forward * 60;  //当MotorR和MotorL相等时，小车为直行状态
                          //小车的转弯通过设置后驱两轮不同速度实现
  if (turn > 0) 
    MotorL = MotorL/turn/5;                      //左转  //根据遥控器程序协议，turn取值为-5~5
  else  if (turn < 0) MotorR = MotorR/(-turn)/5; //右转  //即两后轮最大差速为250-125=125
  myservo.write(90-turn*10);
}
/************************函数功能：避障设置***********************
...
void avoidObstacle()
{
  ...
  if (forward <= 0 && distance_Rear < limit)  Flag_Stop = 2;    //后方路障，小车停止
}
...
void loop()
{
  ...
  if (stringComplete == true)//回传数据内容可以自行定义
  {
    ...
    if(distance_Front<limit||distance_Rear<limit)
      if(distance_Front<limit) Serial.println(1); //Serial.print("-"); 
